...
export function TrialNudgeD7({ city, deal, premiumUrl, prefsUrl, unsubscribeUrl }: { city: string; deal: TrialNudgeDeal | null; premiumUrl: string; prefsUrl: string; unsubscribeUrl: string }) {
  return <Html><Head /><Preview>Free keeps 1 city. Premium watches all 20 and unlocks without the weekly cap.</Preview>
    <Body style={{ backgroundColor: '#FAF7F2', fontFamily: 'Inter, sans-serif', margin: 0 }}><Container style={{ maxWidth: '540px', margin: '0 auto', padding: '32px 20px' }}>
      ...
      <Section style={{ backgroundColor: '#FFFFFF', borderRadius: '12px', padding: '14px 18px', margin: '12px 0' }}><Text style={{ fontWeight: 700, color: '#141210' }}>Premium trial (7 days):</Text><Text style={{ color: '#5C5852' }}>All 20 destinations · Instant + daily alerts · Unlimited unlocks · Filters (discount, stars, price)</Text></Section>
