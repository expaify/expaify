...
describe('booking hotel context continuity', () => {
  ...
  it('round-trips validated hotel stay, score, freshness, and quality evidence', () => {
    const score = {
      percentile: 12,
      pctVsMedian: -31.5,
      medianCents: 27500,
    ...
    const parsed = parseBookingHotelContext(
      Object.fromEntries(new URL(href, 'https://expaify.test').searchParams),
    );
  ...
  it('rejects inconsistent stays, malformed evidence, and unattributed or HTTP provider URLs', () => {
    const baseContext = {
