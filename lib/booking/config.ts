...
function parseStayContext(input: HotelContextInput): Pick<BookingHotelContext, 'checkIn' | 'checkOut' | 'nightCount'> | null {
  ...
  const nightCount = isBlank(input.nightCount) ? undefined : parseInteger(input.nightCount);
  ...
  if (checkIn && checkOut) {
    const calculatedNights = Math.round(
      (Date.parse(`${checkOut}T00:00:00.000Z`) - Date.parse(`${checkIn}T00:00:00.000Z`)) / 86_400_000,
    );
...
function parseRatingEvidence(input: HotelContextInput, prefix: 'hotelClass' | 'guestRating'): HotelRatingEvidence | undefined | null {
  ...
  const reviewCount = isBlank(input[`${prefix}ReviewCount`]) ? undefined : parseInteger(input[`${prefix}ReviewCount`]);
  const sourceLabel = cleanOptional(input[`${prefix}SourceLabel`]);
  const fetchedAt = cleanOptional(input[`${prefix}FetchedAt`]);

  const hasAny = kind !== undefined || confidence !== undefined || value !== undefined ||
    scaleMax !== undefined || reviewCount !== undefined || sourceLabel !== undefined || fetchedAt !== undefined;
...
function parseDealScore(input: HotelContextInput, observedCurrency: string): DealScore | undefined | null {
  const fields = [
    input.scorePercentile,
    input.scorePctVsMedian,
    input.scoreMedianCents,
    input.scoreCurrency,
    input.scoreVerdict,
    input.scoreConfidence,
...
function addRatingEvidenceParams(
  params: URLSearchParams,
  prefix: 'hotelClass' | 'guestRating',
  evidence?: HotelRatingEvidence,
): void {
  if (!evidence) return;
