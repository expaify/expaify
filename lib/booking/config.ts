...
import { notProvidedHotelDocumentReadiness } from '../providers/hotelDocumentReadiness';
import {
  hasValidCoordinates,
...
export type BookingHotelContext = {
  ...
  // Retained for the document-readiness route. Detail presentation does not
  // make document claims as part of the room-inspection decision sequence.
...
export function validateBookingHotelContext(input: HotelContextInput): BookingHotelContext | null {
  ...
  return {
    ...
    documentReadiness: notProvidedHotelDocumentReadiness(providerEvidenceLabel(provider)),
  };
}
