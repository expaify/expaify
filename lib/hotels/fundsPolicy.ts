...
function recordsAreIncompatible(
  ...
  const leftFacts = {
    amount: left.amount,
    basis: left.basis,
    applicationWording: left.applicationWording,
    paymentMethodWording: left.paymentMethodWording,
    returnOrRelease: left.returnOrRelease,
  };
  const rightFacts = {
    amount: right.amount,
    basis: right.basis,
    applicationWording: right.applicationWording,
    paymentMethodWording: right.paymentMethodWording,
    returnOrRelease: right.returnOrRelease,
  };
  const sameFacts = JSON.stringify(leftFacts) === JSON.stringify(rightFacts);

  if (left.type !== right.type) {
    // Different mechanisms can coexist. Treat them as incompatible only when
    // every other normalized fact identifies the same obligation.
    return sameFacts;
  }
  return !sameFacts;
}
...
export function normalizeHotelFundsPolicyEvidence(
  ...
  const conflictRecords = Array.isArray(input.conflictingRecords)
    ...
    : [];
  const hasIncompatiblePair = conflictRecords.some((candidate, index) => (
    conflictRecords.slice(index + 1).some(other => recordsAreIncompatible(candidate.record, other.record))
  ));
  if (state === 'conflicting' && hasIncompatiblePair) {
    return {
      state,
