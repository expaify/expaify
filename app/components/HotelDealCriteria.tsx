...
export function roomHandoffVisibilityTransition(
  ...
  if (visibility === 'hidden' && state === 'armed') return 'away'
...
function readRoomHandoffSession(key: string): RoomHandoffSession | null {
  try {
    ...
    if (
      ...
      (parsed.awayStartedAt !== undefined && (!Number.isFinite(parsed.awayStartedAt) || parsed.awayStartedAt < 0)) ||
...
export function roomHandoffAwayDurationBucket(awayStartedAt: number | undefined, returnedAt: number):
  'under_10s' | '10s_59s' | '1m_4m' | '5m_29m' | '30m_plus' | 'unknown' {
  if (awayStartedAt === undefined || !Number.isFinite(awayStartedAt) || returnedAt < awayStartedAt) return 'unknown'
  const elapsed = returnedAt - awayStartedAt
  if (elapsed < 10_000) return 'under_10s'
  if (elapsed < 60_000) return '10s_59s'
  if (elapsed < 300_000) return '1m_4m'
  if (elapsed < 1_800_000) return '5m_29m'
  return '30m_plus'
}
...
export function HotelDealCriteriaHandoff({ context, deal, links, hotelName, datesIncomplete, disruptionEvidence = NO_HOTEL_DISRUPTION_EVIDENCE, disruptionFixture = false }: {
  ...
  const contextRestorationStatus = status === 'matched'
    ? criteria ? 'restorable' : 'missing'
    : status

  function inventoryAnalyticsProps() {
    return {
  ...
  function beginRoomHandoff(provider: keyof CompareLinks, href: string): void {
    ...
    track('hotel_room_handoff_started', {
      ...
      provider,
      deal_id: deal.id,
      ...(criteria ? { criteria_version: criteria.criteriaVersion } : {}),
      context_status: status,
      destination_present: criteria?.destination.state === 'selected',
      date_state: criteria?.dates.semantic ?? 'missing',
  ...
  function emitRoomHandoffReturn(session: RoomHandoffSession): void {
    track('hotel_room_handoff_returned', {
      handoff_session_id: session.id,
      provider: session.provider,
      deal_id: deal.id,
      away_duration_bucket: roomHandoffAwayDurationBucket(session.awayStartedAt, Date.now()),
      ...inventoryAnalyticsProps(),
    })
    track('hotel_room_recovery_viewed', {
      handoff_session_id: session.id,
      deal_id: deal.id,
      context_restoration_status: contextRestorationStatus,
      ...inventoryAnalyticsProps(),
    })
  }
  ...
  useEffect(() => {
    ...
    if (restored) {
      if (restored.state === 'away' && document.visibilityState === 'visible') {
        ...
        emitRoomHandoffReturn(returned)
      } else {
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                <a
                  ...
                  onClick={(event) => {
                    ...
                    track('hotel_room_recovery_action', {
                      handoff_session_id: active.id,
                      deal_id: deal.id,
                      action: 'recheck_same_hotel',
                      context_restoration_status: contextRestorationStatus,
                      ...inventoryAnalyticsProps(),
                    })
                    const restarted: RoomHandoffSession = {
                      id: crypto.randomUUID(),
                      provider: active.provider,
                      href: active.href,
                      state: 'armed',
                    }
                    activeRoomHandoff.current = restarted
                    setShowRoomRecovery(false)
                    storeRoomHandoffSession(roomHandoffKey, restarted)
                    track('hotel_room_handoff_restarted', {
                      prior_handoff_session_id: active.id,
                      handoff_session_id: restarted.id,
                ...
                <a
                  ...
                  onClick={() => {
                    ...
                    track('hotel_room_recovery_action', {
                      handoff_session_id: active.id,
                      deal_id: deal.id,
                      action: 'back_to_matching_hotels',
                      context_restoration_status: contextRestorationStatus,
                      ...inventoryAnalyticsProps(),
                    })
                  }}
                  className="btn btn-outline inline-flex min-h-11 w-full items-center justify-center text-center sm:w-auto"
                >{resultsLabel}</a>
              </div>
            </div>
          ) : null}
