    ...
    <>
      ...
      <main className="reveal-scope">
        ...
        <section className="relative mx-auto max-w-[1200px] px-5 py-16 min-[1024px]:py-24">
          ...
          <div className="relative z-[1] grid items-center gap-12 min-[1024px]:grid-cols-[0.45fr_0.55fr] min-[1024px]:gap-16">
            <div className="max-w-[520px]">
              <Reveal delayMs={0}><p className="mb-5 text-small font-semibold uppercase tracking-[0.08em] text-[color:var(--primary)]">{marketCount} cities · 60-day median · 30%+ only</p></Reveal>
              <Reveal delayMs={90}><h1 className="text-display text-[color:var(--ink)] min-[640px]:text-[52px]">Never overpay for a hotel again.</h1></Reveal>
              <Reveal delayMs={180}><p className="text-body mt-6 text-[color:var(--ink-soft)]">We watch hotel prices daily and flag the moment a stay drops 30%+ below normal.</p></Reveal>
              <Reveal delayMs={270}><div className="mt-8 flex flex-wrap items-center gap-5">
        ...
        <section className="mx-auto max-w-[1200px] px-5 py-16 min-[1024px]:py-24">
          <Reveal><div className="mb-8 flex flex-wrap items-end justify-between gap-4"><div><p className="text-small font-semibold uppercase tracking-[0.08em] text-[color:var(--primary)]">Verified drops</p><h2 className="text-h2 mt-2 text-[color:var(--ink)]">Live deals right now</h2></div><div className="flex items-center gap-4"><a href="https://www.instagram.com/expaify" target="_blank" rel="noopener noreferrer" className="text-small font-semibold text-[color:var(--ink-soft)] hover:text-[color:var(--primary)]">Follow for daily deals ↗</a><a href="/deals" className="text-small font-semibold text-[color:var(--primary)]">See all →</a></div></div></Reveal>
          {deals.length ? <div className="grid gap-6 min-[680px]:grid-cols-2 min-[1024px]:grid-cols-3">{deals.slice(0, 3).map((deal, i) => <Reveal key={deal.id} delayMs={i * 100}><DealCard deal={deal} href="/deals" /></Reveal>)}</div> : <p className="text-body text-[color:var(--ink-soft)]">No verified live deals at this moment. Check back after the next price scan.</p>}
        </section>
        ...
        <section className="border-y border-[color:var(--line-ivory)] bg-[color:var(--surface)] py-16 min-[1024px]:py-24">
          <div className="mx-auto max-w-[1200px] px-5"><h2 className="text-h2 text-[color:var(--ink)]">Explore destinations</h2><p className="text-body mt-3 text-[color:var(--ink-soft)]">See the places we scan for hotel price drops every day.</p>
            <div className="-mx-5 mt-8 flex snap-x gap-4 overflow-x-auto px-5 pb-3 min-[1024px]:mx-0 min-[1024px]:grid min-[1024px]:grid-cols-8 min-[1024px]:overflow-visible min-[1024px]:px-0">
              {featured.map((market, i) => <Reveal key={market.city} delayMs={Math.min(i * 60, 420)}><a href={`/destinations/${CITY_DISPLAY_TO_SLUG[market.city]}`} className="group relative aspect-[4/5] w-[180px] flex-none snap-start overflow-hidden rounded-[var(--radius-card)] bg-[color:var(--primary)] shadow-[var(--shadow-card-rest)] transition-[transform,box-shadow] duration-200 hover:-translate-y-0.5 hover:shadow-[var(--shadow-card-hover)] min-[1024px]:w-auto"><img src={market.photoUrl} alt={market.photoAlt} loading="lazy" className="h-full w-full object-cover transition-transform duration-200 group-hover:scale-[1.02] motion-reduce:transition-none" /><span className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent" aria-hidden /><span className="absolute inset-x-0 bottom-0 p-4 font-display text-body font-bold text-white">{market.city}</span></a></Reveal>)}
            </div>
        ...
        ].map(([n,title,body], i) => <Reveal key={n} delayMs={i * 120}><span className="text-numeral" aria-hidden>{n}</span><h3 className="text-h3 mt-5 text-[color:var(--ink)]">{title}</h3><p className="text-body mt-3 text-[color:var(--ink-soft)]">{body}</p>{n === '02' ? <div className="mt-6 w-40 space-y-2 text-caption text-[color:var(--ink-soft)]" aria-label="Example comparison: median price is higher than the current price"><div className="flex items-center gap-2"><span className="w-12">Median</span><Reveal as="span" className="reveal-bar h-3 w-24 rounded-[var(--radius-pill)] bg-[color:var(--primary-soft)]" style={{ '--bar-target': '96px' } as CSSProperties} /></div><div className="flex items-center gap-2"><span className="w-12">Now</span><Reveal as="span" className="reveal-bar h-3 w-14 rounded-[var(--radius-pill)] bg-[color:var(--accent)]" style={{ '--bar-target': '56px' } as CSSProperties} /></div></div> : null}</Reveal>)}</div></section>
        ...
        <section id="faq" className="py-16 min-[1024px]:py-24"><div className="mx-auto max-w-[640px] px-5"><Reveal><h2 className="text-h2 mb-10 text-center text-[color:var(--ink)]">Questions</h2></Reveal><FaqAccordion /></div></section>
      </main>
    </>
  )
}
