  ...
  const content = (
    <article className={`group overflow-hidden rounded-[var(--radius-card)] border-[0.5px] border-[color:var(--line-ivory)] bg-[color:var(--surface)] ${deal.expired ? 'grayscale' : deal.isMock ? '' : 'transition-[transform,box-shadow] duration-150 hover:-translate-y-1 hover:shadow-[var(--shadow-card-hover)]'}`}>
      <div className="space-y-3 px-4 pb-4 pt-3">
        <div>
          {deal.isMock ? (
            <span className="mb-2 inline-flex rounded-[var(--radius-pill)] bg-[color:var(--bg-muted)] px-2 py-1 font-display text-[11.5px] font-bold leading-none text-[color:var(--ink-soft)]">
              Example
            </span>
          ) : null}
          <h3 className="line-clamp-2 font-display text-[15px] font-bold leading-snug text-[color:var(--ink)]">
            {deal.hotelName}
          </h3>
          <p className="mt-0.5 text-[11.5px] leading-snug text-[color:var(--ink-faint)]">
        ...
        <div className="space-y-2">
          <div className="flex min-w-0 flex-wrap items-baseline gap-2">
            <span className="font-display text-[30px] font-bold leading-none text-[color:var(--primary)]">
              {formatMoney(deal.dealPrice)}
            </span>
            <span className="self-end pb-0.5 text-[11.5px] leading-none text-[color:var(--ink-faint)]">/ night</span>
            <span className="text-[13px] leading-none text-[color:var(--ink-faint)] line-through">
              usually {formatMoney(deal.medianPrice)}
            </span>
            {deal.expired ? (
              <span className="inline-flex items-center rounded-[var(--radius-pill)] bg-[color:var(--bg-muted)] px-3 py-1.5 font-display text-[13px] font-bold leading-none text-[color:var(--ink-soft)]">
                Expired
              </span>
            ) : (
          ...
          {deal.headline ? (
            <p className="text-[11.5px] font-medium leading-snug text-[color:var(--primary)]">{deal.headline}</p>
          ) : null}
          {showSavings ? (
            <p className="text-[11.5px] font-medium text-[color:var(--primary)]">
        ...
        {deal.expired ? null : deal.isMock ? (
          ...
          <p className="flex min-h-11 items-center justify-center rounded-[var(--radius-input)] border border-[color:var(--primary)] text-[13px] font-medium text-[color:var(--primary)]">View deal</p>
        ) : (
