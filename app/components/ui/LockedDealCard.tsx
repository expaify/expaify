  ...
  return (
    <article className="overflow-hidden rounded-[var(--radius-card)] border-[0.5px] border-[color:var(--line-ivory)] bg-[color:var(--surface)] transition-[transform,box-shadow] duration-150 hover:-translate-y-1 hover:shadow-[var(--shadow-card-hover)]">
      <div className="flex flex-wrap items-center gap-2 px-4 pt-4">
        <span className="rounded-[var(--radius-pill)] bg-[color:var(--primary)] px-3 py-1 font-display text-[15px] font-bold leading-none text-[color:var(--text-inverse)]">
          Members
        </span>
        <span className="rounded-[var(--radius-pill)] border border-[color:var(--border)] bg-[color:var(--bg-surface)] px-2 py-1 text-[11.5px] font-medium leading-none text-[color:var(--text-2)]">
          Deal found today
        </span>
      </div>
      ...
      <div className="relative space-y-3 px-4 pb-4 pt-3">
        <div className="pointer-events-none select-none blur-[5px]" aria-hidden>
          <h3 className="font-display text-[15px] font-bold leading-snug text-[color:var(--ink)]">{placeholderName}</h3>
        ...
        <div className="pointer-events-none select-none blur-[5px]" aria-hidden>
          <div className="grid grid-cols-4 gap-2">
            {['Expedia', 'Booking', 'Kiwi', 'Trip.com'].map(name => (
              <div key={name} className="rounded-[var(--radius-input)] border-[0.5px] border-[color:var(--line-white)] py-2 text-center text-[11.5px] font-medium text-[color:var(--ink)]">
                {name}
              </div>
            ))}
          </div>
        ...
        <div className="absolute inset-x-4 top-1/2 flex -translate-y-1/2 flex-col items-center gap-3 rounded-[var(--radius-input)] bg-[color:var(--bg-overlay)] px-4 py-4 text-center shadow-[var(--shadow-card-hover)] backdrop-blur-[2px]">
          ...
          <p className="text-[11.5px] font-medium text-[color:var(--ink-soft)]">Members-only deal</p>
          <a href={joinHref} className="btn btn-conversion min-h-9 px-5 text-[13px]">Unlock with Premium</a>
        </div>
      </div>
    </article>
  )
