...
describe('hotel decision analytics', () => {
  ...
  it('emits provider handoff and explicit back events without raw URLs', () => {
    const tree = HotelDecisionAnalytics({
      hotelId: 'hotel_123',
      ...
      children: null,
    }) as ReactElement<{ onClick: (event: unknown) => void }>

    tree.props.onClick({
      target: {
        closest: (selector: string) => selector === '[data-hotel-provider]'
          ? { dataset: { hotelProvider: 'hotellook' } }
          : null,
      },
    })
    expect(trackMock).toHaveBeenCalledWith(
      'hotel_room_handoff_started',
      expect.objectContaining({ hotel_id: 'hotel_123', provider: 'hotellook' }),
    )

    tree.props.onClick({
      target: {
        closest: (selector: string) => selector === '[data-hotel-back]' ? {} : null,
      },
    })
    expect(trackMock).toHaveBeenCalledWith(
      'hotel_detail_back_to_results',
      expect.objectContaining({ hotel_id: 'hotel_123', entry_source: 'hotel_results' }),
    )
    expect(JSON.stringify(trackMock.mock.calls)).not.toContain('https://')
  })
})
