...
describe('QuietStayEvidenceLedger', () => {
  ...
  it('adds the bounded cue to the deal link alongside the destination link', () => {
    ...
    expect(populated).toContain('aria-label="View hotel: Example Hotel. 4-star hotel class. Quiet-stay evidence available · Property."')
  ...
  it('mounts the one production ledger inside Hotel fit before provider handoff', () => {
    ...
    expect(card.indexOf('{winningCue ? (')).toBeGreaterThan(card.indexOf("{' · '}<DealCardCity city={deal.city} />"))
    expect(card.indexOf('{winningCue ? (')).toBeLessThan(card.indexOf('<div className="mt-auto space-y-2">'))
  })
})
