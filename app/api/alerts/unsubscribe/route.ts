...
function html(message: string, status = 200): NextResponse {
...
export async function GET(req: Request): Promise<NextResponse> {
  ...
  if (!/^[0-9a-fA-F-]{36}$/.test(token)) {
    return html('This unsubscribe link is invalid. You can still manage alerts from your account page.', 400)
  }
  ...
  if (!result || result.rowCount === 0) {
    return html('This unsubscribe link has expired or was already removed. You can still manage alerts from your account page.', 404)
  }

  return html('You will no longer receive expaify deal alerts. Transactional account and billing emails may still be sent.')
}
