...
describe('BookingFlow fare context review', () => {
  ...
  it('renders aging and stale price checks with their specified warning copy', () => {
    const nowSpy = jest.spyOn(Date, 'now').mockReturnValue(Date.parse('2026-07-26T00:00:00.000Z'));

    try {
      const agingText = collectText(BookingFlow({
        bookingEnabled: false,
        duffelSandbox: false,
        fareContext: null,
        hotelContext: { ...hotelContext, priceCheckedAt: '2026-07-24T17:00:00.000Z' },
      }));
      const staleText = collectText(BookingFlow({
        bookingEnabled: false,
        duffelSandbox: false,
        fareContext: null,
        hotelContext: { ...hotelContext, priceCheckedAt: '2026-07-23T00:00:00.000Z' },
      }));

      expect(agingText).toContain('Price checked 31 hours ago. Confirm the current rate with the provider.');
      expect(staleText).toContain('Price may be out of date. We have not rechecked it since Jul 23, 2026.');
      expect(staleText).not.toContain('Price checked Jul 23, 2026.');
    } finally {
      nowSpy.mockRestore();
    }
  });

  it('distinguishes provider-only and inferred ratings from absent guest evidence', () => {
    const providerOnlyText = collectText(BookingFlow({
      bookingEnabled: false,
      duffelSandbox: false,
      fareContext: null,
      hotelContext: {
    ...
    const inferredText = collectText(BookingFlow({
      bookingEnabled: false,
      duffelSandbox: false,
      fareContext: null,
      hotelContext: {
        ...hotelContext,
        guestRating: { kind: 'inferred', confidence: 'inferred', value: 4, scaleMax: 5 },
      },
    }));

    expect(providerOnlyText).toContain('8.4/10 provider rating from Hotellook');
    expect(providerOnlyText).toContain('Review source not confirmed.');
    expect(providerOnlyText).not.toContain('Guest rating not provided');
    expect(inferredText).toContain('No verified guest rating');
    expect(inferredText).toContain('We do not label inferred hotel data as a guest rating.');
    expect(inferredText).not.toContain('Guest rating not provided');
  });

  it('does not give the incomplete-date provider instruction for a complete stay', () => {
    const text = collectText(BookingFlow({
      bookingEnabled: false,
      duffelSandbox: false,
      fareContext: null,
    ...
    expect(text).toContain('Rate shown for this stay context; the provider confirms room-level details.');
    expect(text).not.toContain('Choose or confirm your dates there before comparing room options.');
  });
