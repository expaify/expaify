...
| **Codex CLI** (`codex exec --sandbox workspace-write`) | Implementer for multi-file/mechanical changes needing real tsc/test execution; verifier (runs the actual gate) | ChatGPT OAuth, `~/.codex/auth.json` | Flat subscription, not metered per-call | 🟢 alive | 2026-08-14 |
| **Gemini** (`gemini-3.5-flash`) | Persona-stage docs (UXD/UXR/UXDES) — lighter text generation, not code diffs | `GEMINI_API_KEY` at `~/.config/gemini/credentials` | Metered | 🟢 alive | 2026-08-14 |
